...
describe("PUT /api/characters/:id", () => {
  ...
  test("401 sem Authorization", async () => {
    const req = makeNextJsonRequest("http://localhost/api/characters/x", "PUT", {
  ...
  test("404 quando character não existir", async () => {
    ...
    const token = await makeToken({ sub: ownerId, email: ownerEmail, name: "O" });
  ...
  test("409 quando status não é editável (ex: SUBMITTED)", async () => {
    ...
    const token = await makeToken({ sub: ownerId, email: ownerEmail, name: "Owner" });
