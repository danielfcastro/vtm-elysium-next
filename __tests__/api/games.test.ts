...
import { cleanupTestArtifacts, makeRunTag, seedTestUser } from "../helpers/testDb";
...
describe("GET/POST /api/games", () => {
    ...
    afterAll(async () => {
        await cleanupTestArtifacts({ gameIds: createdGameIds, userEmails: createdUserEmails });
        await pool.end();
    });
    ...
    test("POST 201 cria game e adiciona STORYTELLER em user_game_roles; GET lista o game", async () => {
        ...
        const getReq = makeNextJsonRequest("http://localhost/api/games", "GET", undefined, {
            Authorization: `Bearer ${token}`,
        });
        ...
        expect(getJson.games.some((g: any) => g.id === postJson.game.id)).toBe(true);
    });
});
