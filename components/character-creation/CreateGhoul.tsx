...
import { GhoulDisciplineLimitStrategy } from "@/core/strategies/GhoulDisciplineLimitStrategy";
...
const HUMANITY_FREEBIE_COST = new FreebiePointCostStrategy().getCost(
  TraitType.Humanity,
);
...
export function CreateGhoul({
  ...
  const maxDisciplineLevel = useMemo(() => {
    if (ghoulOptions?.isGhoul) {
      return GhoulDisciplineLimitStrategy.getMaxDisciplineLevel(
        ghoulOptions.domitorGeneration,
      );
    }
    return 5;
  }, [ghoulOptions]);
  ...
  const [draft, setDraft] = useState<CharacterDraft>(() => {
    const d = createEmptyCharacterDraft({
      ...
      domitorClan: ghoulOptions?.domitorClan,
      domitorGeneration: ghoulOptions?.domitorGeneration,
      maxDiscipline: maxDisciplineLevel,
    });
    if (gameName) d.chronicle = gameName;
    return d;
  });
  const [phase, setPhase] = useState<CreationPhase>(1);
  const [templateKey, setTemplateKey] = useState<TemplateKey>("ghoul");
  const [backgroundRows, setBackgroundRows] = useState<TraitRow[]>(() =>
    createRowsFromRecord(
      createEmptyCharacterDraft({ isGhoul: true }).backgrounds,
    ),
  );
  const [disciplineRows, setDisciplineRows] = useState<TraitRow[]>(() =>
    createRowsFromRecord(
      createEmptyCharacterDraft({ isGhoul: true }).disciplines,
    ),
  );
    ...
    if (initialCharacterId && initialCharacterId !== prevCharacterId) {
      ...
      setDraft(
        createEmptyCharacterDraft({
          ...
          domitorClan: ghoulOptions?.domitorClan,
          domitorGeneration: ghoulOptions?.domitorGeneration,
          maxDiscipline: ghoulOptions?.maxDiscipline,
        }),
      );
      setTemplateKey("ghoul");
      setBackgroundRows(
        createRowsFromRecord(
          createEmptyCharacterDraft({ isGhoul: true }).backgrounds,
        ),
  ...
  const disciplineOptions = useMemo(() => {
    ...
    // Bug #7b: a Ghoul can only learn disciplines that his domitor has
    if (domitorDisciplines.length === 0) return all; // Fallback if no clan selected or no disciplines found
    return all.filter((d) => domitorDisciplines.includes(d.id));
  }, [ghoulOptions, domitorDisciplines]);

  const backgroundOptions = useMemo(() => {
    const all = backgroundsJson as NamedItem[];
    if (!ghoulOptions?.isGhoul || ghoulOptions.isRevenant) return all;
    // Bug #6: AGE and GENERATION are unavailable to a ghoul
    return all.filter((bg) => bg.id !== "age" && bg.id !== "generation");
  }, [ghoulOptions]);
  ...
  const disciplineValueLimit = traitCap;
  ...
  function validateTraitCapsForSave(d: CharacterDraft): string | null {
    ...
    for (const b of cappedBuckets) {
      ...
      for (const [_key, v] of Object.entries(b)) {
        const n = Number(v ?? 0);
  ...
  function reconstructPhase1SnapshotsFromCurrent(params: {
    ...
    const { draft, disciplineRows, backgroundRows, rules } = params;
  ...
  function handleLoadSavedDraft() {
    ...
    try {
      ...
      if (nextBgRows) {
        try {
          applyBackgroundsToDraft(parsed.backgroundRows);
        } catch {
          // ignore
        }
      }
  ...
  function _handleToggleDarkAges(_e: React.ChangeEvent<HTMLInputElement>) {
    // No-op for human ghouls
  }
  ...
  const bloodPoolMax = 10;
  const bloodPerTurn = 1;
  const _maxTraitRating = 5;

  const clanWeakness =
    ((clans as any[]).find((clan) => clan.id === draft.domitorClan) as any)
      ?.weakness ?? "—";
            ...
            <div className="sheetActive">
              ...
              <div className="sheetSection">
                ...
                <div className="personaGrid personaGridCreate">
                  {/* ===== ROW 1: Name | Nature | Domitor Clan ===== */}
                  ...
                  <p className="personaRow">
                    ...
                    <AutocompleteInput
                      label=""
                      valueId={draft.domitorClan}
                      onChangeId={(id) => updateDraft({ domitorClan: id })}
                      options={clanOptions}
                      placeholder="Selecione o Clan do Domitor"
                    />
                  </p>

                  {/* ===== ROW 2: Player | Demeanor | Domitor Generation ===== */}
                  <p className="personaRow">
                  ...
                  <p className="personaRow">
                    <strong>Domitor Generation:</strong>{" "}
                    {draft.domitorGeneration ?? "—"}th
                  </p>

                  {/* ===== ROW 3: Chronicle | Concept | Domitor Name ===== */}
                  <p className="personaRow">
                  ...
                  <p className="personaRow">
                    <strong>Domitor Name:</strong>
                    <input
                      className="textInput"
                      value={draft.domitorName ?? ""}
                      onChange={(e) =>
                        updateDraft({ domitorName: e.target.value })
                      }
                      placeholder="Nome do domitor"
                    />
                  </p>
                ...
                {/* Metadados - Max Blood Pool, Blood Per Turn, Max Trait Rating */}
                <div className="personaGrid personaGridCreate hrTop">
                  <p>
                    <strong>Max Blood Pool:</strong> {bloodPoolMax}
                  </p>
                  <p>
                    <strong>Blood Per Turn:</strong> {bloodPerTurn}
                  </p>
                  <p>
                    <strong>Max Trait Rating:</strong> {_maxTraitRating}
                  </p>
                </div>
              </div>
