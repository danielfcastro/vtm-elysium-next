...
const _GENERATION_BACKGROUND_ID = "generation";
...
function DotsSelector({
  ...
  return (
    <span
      className={`dots dotsSelector${disabled ? " dotsDisabled" : ""}`}
...
export function CreateAnimal({
  ...
  useEffect(() => {
    if (ghoulOptions) {
      ...
      setDraft((prev) => ({
        ...
        player: ghoulOptions.domitorPlayer || prev.player,
      }));
    }
  }, [ghoulOptions]);

  const _isRevenant = false;
  ...
  const domitorDisciplines = useMemo(() => {
    ...
    return (clan?.disciplines as string[]) ?? [];
  }, [draft.domitorClan]);

  const disciplineOptions = useMemo(() => {
    const all = disciplinesJson as NamedItem[];
    if (!ghoulOptions?.isGhoul || ghoulOptions.isRevenant) return all;
    if (domitorDisciplines.length === 0) return all;
    return all.filter((d) => domitorDisciplines.includes(d.id));
  }, [ghoulOptions, domitorDisciplines]);
  ...
  // Get max trait value for animal ghouls from template
  const getAnimalTemplateTraitCap = (): number => {
    if (!isAnimalGhoul || !selectedAnimalTemplate) return traitCap;
    const attrs = selectedAnimalTemplate.traits?.attributes || {};
    const abilities = selectedAnimalTemplate.traits?.abilities || {};
    const allValues = [
      ...Object.values(attrs as Record<string, number>),
      ...Object.values(abilities as Record<string, number>),
    ].filter((v) => typeof v === "number");
    const maxTemplateValue = Math.max(...allValues, 0);
    return Math.max(maxTemplateValue, traitCap);
  };

  const animalTraitCap = getAnimalTemplateTraitCap();

  // Calculate discipline cap based on domitor generation for ghouls
  const domitorGen =
    ghoulState?.domitorGeneration ?? draft.domitorGeneration ?? 13;
  const ghoulDisciplineCap =
    domitorGen >= 13 ? 1 : Math.max(1, 14 - domitorGen);
  // Show up to 5 dots but limit the selectable value for ghouls
  const disciplineMax = traitCap; // Display up to traitCap dots
  const disciplineValueLimit =
    ghoulState?.isGhoul || draft.isGhoul
      ? Math.min(ghoulDisciplineCap, traitCap)
      : traitCap; // Limit value for ghouls

  // Helper to get max value from animal template (overall max across all traits)
  const getAnimalTemplateMax = (): number => {
    if (!isAnimalGhoul || !selectedAnimalTemplate) return traitCap;
    const attrs = selectedAnimalTemplate.traits?.attributes || {};
    const abilities = selectedAnimalTemplate.traits?.abilities || {};
    const allValues = [
  ...
  function validateTraitCapsForSave(d: CharacterDraft): string | null {
    ...
    // Get locked traits from animal template (allows higher values)
    const lockedAttrs = (d as any).lockedTraits?.attributes || {};
    const lockedAbilities = (d as any).lockedTraits?.abilities || {};

    // Helper to check if a value is from locked traits
    const isLockedTrait = (
      ...
      if (traitType === "attribute") {
        return lockedAttrs[key] === value;
      }
      return lockedAbilities[key] === value;
    };

    // Traits que obedecem maxTraitRating (skip backgrounds for animal ghouls)
    const cappedBuckets: any[] = [
      (d as any).attributes,
      (d as any).abilities,
      ...(isAnimalGhoul ? [] : [(d as any).backgrounds]),
    ...
    for (const b of cappedBuckets) {
      ...
      for (const [key, v] of Object.entries(b)) {
        ...
        if (n > cap) {
          // Allow if it's a locked trait from animal template
          const traitType =
            b === (d as any).attributes ? "attribute" : "ability";
          if (!isLockedTrait(traitType, key, n)) {
  ...
  function saveDraftToLocalStorage() {
    ...
    try {
      ...
      const _safePhase: CreationPhase =
        phase === 2 && !hasSnapshots ? 1 : phase;
  ...
  function reconstructPhase1SnapshotsFromCurrent(params: {
    ...
    const {
      ...
      backgroundRows,
      rules: _rules,
      isDarkAges: _isDarkAges,
    } = params;

    // Base helpers
    const _clanId = draft.clanId ?? null;
    ...
    // Ensure background-derived stats are consistent
    phase1DraftSnapshot.generation = 13;
  ...
  async function createNewCharacterInDatabase(): Promise<string | null> {
    if (!gameId) {
      setToast("No game selected");
      return null;
    }
    ...
    try {
      const res = await fetch(`/api/games/${gameId}/characters`, {
        method: "POST",
        headers: {
      ...
      if (!res.ok) {
        ...
        setToast(`Failed to create character: ${err.error ?? res.statusText}`);
        return null;
      }

      const data = await res.json();
      const newCharacterId = data.character?.id ?? data.id;
      if (newCharacterId) {
        setDbCharacterId(newCharacterId);
        return newCharacterId;
      }
      return null;
    } catch (e: any) {
      setToast(`Error creating character: ${e?.message ?? String(e)}`);
      return null;
    }
  }
  ...
  async function saveToDatabase(
    ...
    const targetId = characterId ?? dbCharacterId;
    ...
    try {
      const res = await fetch(`/api/characters/${targetId}`, {
        method: "PUT",
        headers: {
          Authorization: `Bearer ${token}`,
          "Content-Type": "application/json",
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    // Skip trait cap validation for animal ghouls (they use template values)
    if (!isAnimalGhoul) {
    ...
    // Save to database - create new character if none exists
    let savedCharacterId = dbCharacterId;
    if (!dbCharacterId && gameId) {
      // Create new character in database
      savedCharacterId = await createNewCharacterInDatabase();
    }
    if (savedCharacterId) {
      await saveToDatabase(statusOverride, savedCharacterId);
    ...
    if (savedCharacterId) {
      ...
      setToast("Character saved to browser (no game selected).");
    } else {
  ...
  if (phase === 1) {
    // For animal ghouls, use willpower from template (already set in draft)
    // For regular characters, calculate from virtues
    if (isAnimalGhoul && selectedAnimalTemplate) {
      willpowerPermanent = Number(
        draft.willpower ??
          selectedAnimalTemplate.traits?.secondary?.willpower ??
          5,
      );
      roadRating = 0; // Animal ghouls don't have Road/Humanity
    } else {
  ...
  const bloodPerTurn = isAnimalGhoul ? 1 : Number(c.bloodPointsPerTurn ?? 0);
  const maxTraitRating = isAnimalGhoul
    ? animalTraitCap
    : Number(c.maxTraitRating ?? 5);

  const clanWeakness = (c as any)?.clan?.weakness ?? "—";

  const draftMerits = (draft as any).merits ?? [];
  const hasHugeSize = draftMerits.some((m: any) => m.name === "Huge Size");
  ...
  const templateHealthLevels = (selectedAnimalTemplate?.combat?.healthLevels ||
    selectedAnimalTemplate?.traits?.secondary?.healthLevels ||
    []) as string[];

  const baseHealthLevels =
    templateHealthLevels.length > 0
      ? templateHealthLevels.map((label) => ({
          label,
          penalty: label.startsWith("-") ? label : "",
        }))
      : defaultHealthLevels;
            ...
            <div className="sheetActive">
              ...
              <div className="sheetSection">
                ...
                <div className="personaGrid personaGridCreate">
                  ...
                  <p className="personaRow">
                    <strong>Race:</strong>
                    <span className="personaValue" style={{ color: "#90ee90" }}>
                      {selectedAnimalTemplate?.name || "-"}
                    </span>
                  </p>

                  {/* ROW 2: Domitor | -- | Chronicle */}
                  <p className="personaRow">
                    <strong>Domitor:</strong>
                    <input
                      className="textInput"
                  ...
                  {nameError && (
                    <p
                      className="personaRowFull fieldError personaFullWidth"
                ...
                {/* Metadados de Geração - show Max Blood Pool for ghouls too */}
                <div className="personaGrid personaGridCreate hrTop">
                  <p>
                    <strong>Max Blood Pool:</strong>{" "}
                    {c.maximumBloodPool ?? (ghoulState?.isGhoul ? 10 : "—")}
                  </p>
                  <p>
                    <strong>Blood Per Turn:</strong>{" "}
                    {bloodPerTurn ?? (ghoulState?.isGhoul ? 1 : "—")}
                  </p>
                  <p>
              ...
              <div className="sheetSection">
                ...
                <div className="grid3">
                  ...
                  <div>
                    ...
                    {disciplineRows.map((row) => (
                      <div className="itemRow" key={row.key}>
                        ...
                        <DotsSelector
                          value={Math.min(row.dots, disciplineValueLimit)}
                          max={disciplineMax}
                          onChange={(dots) =>
                            handleDisciplineDotsChange(
                              row.key,
                              Math.min(dots, disciplineValueLimit),
                            )
                          }
                        />
                      </div>
                  ...
                  {/* Virtues - Hidden for Animal Ghouls */}
                  {!ghoulState?.isGhoul && (
                    <div>
              ...
              <div className="sheetSection">
                ...
                <div className="grid3">
                  ...
                  <div>
                    ...
                    {!isAnimalGhoul && !ghoulState?.isGhoul && (
                      <>
                        <h3 className="h3 othersRoadSpacing">
                          Road: {roadName}
                        </h3>
                        <DotsSelector
                          value={roadRating}
