...
import { SheetAttributes, SheetAbilities, SheetHealthLevels } from "./shared";
import {
  disciplineService,
  ComboDiscipline,
  DisciplinePower,
...
const AnimalGhoulCharacterSheet: React.FC<CharacterSheetProps> = ({
  mode,
  sheet,
  onSubmit,
  characterStatus,
  ...
  const _generation: number | undefined = draft.generation;
  const playerName: string = draft.player ?? "";
  const chronicle: string = draft.chronicle ?? "";
  const _sire: string = draft.sire ?? "";
  const _natureId: string = draft.natureId ?? "";
  const _demeanorId: string = draft.demeanorId ?? "";
  const _conceptId: string = draft.conceptId ?? "";

  // Animal template info
  const _animalTemplateId: string =
    draft.animalTemplateId ?? sheetWrapper.animalTemplateId ?? "";
  const animalTemplateName: string =
    draft.animalTemplateName ?? sheetWrapper.animalTemplateName ?? "";

  // Domitor info for ghouls
  const domitorName: string =
    draft.domitorName ?? sheetWrapper.domitorName ?? "";
  const _domitorClan: string =
    draft.domitorClan ?? sheetWrapper.domitorClan ?? "";
  const _domitorGeneration: number | undefined =
    draft.domitorGeneration ?? sheetWrapper.domitorGeneration;
  ...
  const lockedTraits = draft.lockedTraits ?? sheetWrapper.lockedTraits ?? null;
  ...
  // Health levels from template for animal ghouls
  const combatData = sheetWrapper.combat;
  const animalHealthLevels =
    combatData?.healthLevels && Array.isArray(combatData.healthLevels)
      ? combatData.healthLevels.map((hl: string, idx: number) => ({
          id: `hl-${idx}`,
          label: hl,
          penalty: idx === 0 ? 0 : parseInt(hl) || 0,
        }))
      : [];
        ...
        <>
          ...
          <section className="sheetSection">
            ...
            <div className="personaGrid personaGridCreate">
              ...
              <p className="personaRow">
                <strong>--</strong>
                <span className="personaValue">-</span>
              </p>
              ...
              {/* Linha 3: Chronicle, empty, Domitor */}
              <p className="personaRow">
              ...
              <p className="personaRow">
                ...
                <span className="personaValue">
                  {domitorName && domitorName.length > 0 ? domitorName : "-"}
                </span>
              </p>
            </div>
          </section>
          ...
          <section className="sheetSection">
            ...
            {/* Revenant rules bottom line - only for human ghouls with family */}
            {familyName && (
              <div
                style={{
                  marginTop: 16,
          ...
          <section className="sheetSection">
            <div className="grid3">
              ...
              <div>
                ...
                <SheetHealthLevels healthLevels={animalHealthLevels} />
              </div>
            </div>
          </section>
        </>
...
export default AnimalGhoulCharacterSheet;
