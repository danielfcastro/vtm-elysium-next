...
export default function XpRevenantGhoulDrawer({
  isOpen,
  onClose,
  sheet,
  baseAvailableXp,
  ...
  const availableFamilyDisciplines = Object.keys(familyDisciplines).filter(
    (key) => {
      const val = familyDisciplines[key];
      return typeof val === "number" ? val > 0 : ((val as any)?.level ?? 0) > 0;
    },
  );

  const isFamilyDiscipline = (discKey: string): boolean => {
    const keyLower = discKey.toLowerCase();
    return availableFamilyDisciplines.some((d) => d.toLowerCase() === keyLower);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setChanges({});
      setChangesSpecialties({}); // Clear specialty changes as well
      onClose();
    } catch (err) {
